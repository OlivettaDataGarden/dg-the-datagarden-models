...
class ResearchAndDevelopmentInvestment(DataGardenSubModel):
	public_investment: Optional[float] = Field(
		default=None, description=RDI.PUBLIC_INVESTMENT
	)
	company_expenditure: Optional[float] = Field(
		default=None, description=RDI.COMPANY_EXPENDITURE
	)
...
class InvestmentLegends:
	INWARD_FOREIGN_DIRECT_INVESTMENT = (
		"Value of foreign direct investments (FDI) into the country "
		"from non-resident companies."
	)
	OUTWARD_FOREIGN_DIRECT_INVESTMENT = (
		"Value of foreign direct investments (FDI) of resident companies "
		"in other countries."
	)

	RESEARCH_AND_DEVELOPMENT_INVESTMENT = (
		"Value of research and development (R&D) investments."
	)
...
class Investment(DataGardenSubModel):
	...
	research_and_development_investment: Optional[ResearchAndDevelopmentInvestment] = (
		Field(default=None, description=IL.RESEARCH_AND_DEVELOPMENT_INVESTMENT)
	)
