...
class EconomicsV1Legends(DataGardenModelLegends):
	...
	ECONOMICS_METADATA = (
		"Metadata about currency, units and reference year used for the data."
	)
...
class EconomicsV1(DataGardenModel):
	...
	economics_metadata: EconomicsMetaData = Field(
		default_factory=EconomicsMetaData, description=L.METADATA
	)
	...
	productivity: Productivity = Field(
		default_factory=Productivity, description=L.PRODUCTIVITY
	)
	...
	class Meta(DataGardenModel.Meta):
		fields_to_include_in_data_dump: list[str] = (
			DataGardenModel.Meta.fields_to_include_in_data_dump + ["economics_metadata"]
		)
