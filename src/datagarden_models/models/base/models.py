...
class DataGardenSubModel(BaseModel):
	"""
	Base model class for DataGarden models that provides common functionality
	...
	- Converting model data to a cleaned dictionary format with only
	  (sub) models/attributes that have values (data_dump method)
	...
	Attributes:
	    Meta: Inner class for configuration, containing:
	        exclude_fields_in_has_values_check (list[str]): Fields to exclude
	        when checking if model has values
	...
	class Meta:
		...
		fields_to_include_in_data_dump: list[str] = [
...
class DataGardenModel(DataGardenSubModel):
	...
	metadata: MetadataModel = Field(
		default=MetadataModel(), description=DataGardenModelLegends.METADATA
	)
