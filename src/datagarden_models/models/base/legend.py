...
class Legend:
	...
	model : type[DataGardenSubModel]
	    ...
	    Data model population : Population indicators for the region.  (with attributes
	        [by_age_gender, total, total_male, total_female,
	        male_to_female_ratio, density, change, natural_change,
	        natural_change_rate])
	...
	def sub_models(self) -> list["Legend"]:
		return [
			legend for legend in self.field_legends.values() if legend.field_legends
		]

	@property
	def non_sub_model_attributes(self) -> list["Legend"]:
		return [
			legend for legend in self.field_legends.values() if not legend.field_legends
		]
	...
	def show_summary(self):
		...
		if "value_attributes" in summary:
			max_length = (
				max(
					len(attribute_name)
					for attribute_name in summary["value_attributes"].keys()
				)
				+ 2
			)
			...
			for attribute_name, attribute_legend in summary["value_attributes"].items():
				spaces = (max_length - len(attribute_name)) * " "
				print(f"{attribute_name}{spaces} : {attribute_legend}")
