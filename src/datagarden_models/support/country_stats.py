...
class RegionData(BaseModel):
	...
	def model_post_init(self, __context) -> None:
		...
		self.regional_data_stats = {
			k.lower(): v for k, v in self.regional_data_stats.items()
		}
...
class CountryStats(RootModel[Dict[str, RegionData]]):
	...
	def statistics_for_data_model(
		self, model_name: str
	) -> Dict[str, RegionalDataStats]:
		"""Get regional statistics for a specific data model.

		Regional statistics include data avaialability statistics for a single
		data model for all regions.
		...
		if model_name not in self.regional_data_models:
			raise ValueError(
				f"Data model '{model_name}' not found in regional data models"
			)
