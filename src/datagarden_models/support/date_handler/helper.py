...
def utc_datetime(dt: date | datetime) -> datetime:
	...
	"""
	# not isinstance(dt, datetime) is needed as datetime objects are also
	# instances of date
	...
	if not isinstance(dt, datetime):
		raise TypeError(
			f"Invalid date format: {dt}, dt needs to be of type datetime, date or str"
		)
...
class PeriodTransformer(ABC):
	"""
	Public methods:
	    period_start_datetime: returns first day for period of a given period
	        type in datetime format.
	    period_start_str: returns first day for period of a given period type
	        in string format.
	    date_to_period_type_str: returns str representation of a period for
	        a given period type.
	...
	def period_start_datetime(self, period: str | date | datetime) -> datetime:
		...
		if isinstance(period, str):
			...
			if not converted_period:
				raise ValueError(
					f"Could not create {self.PERIOD_TYPE} date from {period}"
				)
		else:
