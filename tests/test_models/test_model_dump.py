...
def test_my_second_data_dump_empty():
	"""
	This test is to check if the data_dump method is working correctly and
	does not include empty sub models
	"""
	model = MySecondModel()
...
def test_my_second_model_dump_empty():
	"""
	This test is to check if the model_dump method is working correctly and
	includes empty sub models
	"""
	model = MySecondModel()
...
def test_my_second_data_dump_has_economics_datal():
	...
	assert data_dump == {
		"my_sub_model": {"metric": {"currency": "EUR", "unit": 1, "value": 1.0}}
	}
